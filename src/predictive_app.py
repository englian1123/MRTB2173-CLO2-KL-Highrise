...
def get_mukim_scheme_mapping():
    try:
        # Load the CSV to get unique Mukim and Scheme relationships
        df = pd.read_csv(csv_file_path)
        # Create a mapping of Mukim to a sorted list of unique SchemeNames
        mapping = df.groupby('Mukim')['SchemeName'].unique().apply(lambda x: sorted(list(x))).to_dict()
        return mapping
    except Exception as e:
        st.error(f"Error loading 'KLHighRise.csv' for dropdowns: {e}")
        return {}
...
@st.cache_resource
def load_models():
    # v1: Basic (no scheme) | v2: Advanced (with scheme target encoding)
...
try:
    ...
    st.error(f"Error loading models: {e}. Ensure the .joblib files are in the same folder.")
    st.stop()
...
if mukim_scheme_map:
    ...
    selected_mukim = st.sidebar.selectbox("Select Mukim", ['Kuala Lumpur Town Centre', 'Mukim Batu', 'Mukim Cheras', 'Mukim Kuala Lumpur', 'Mukim Petaling', 'Mukim Ampang', 'Mukim Setapak', 'Mukim Ulu Kelang'])
...
if st.sidebar.button("Predict Price"):
    ...
    # Prepare data for Model V2 (Advanced)
...
if st.session_state.history:
    ...
    with col1:
        ...
        st.caption(f"General estimate for {latest['Mukim']}")
    ...
    with col2:
        ...
        st.caption(f"Specific estimate for {latest['SchemeName']}")
...
if st.session_state.history:
    ...
    st.info("Adjust the sidebar parameters and click 'Predict Price' to see results.")