...
# Features and Target
# Adding SchemeName to features
features = ['SchemeName', 'Mukim', 'Tenure', 'ParcelArea', 'UnitLevel_Cleaned', 'Year', 'Month']
X = df[features]
...
# Define Preprocessor
categorical_ohe = ['Mukim', 'Tenure']
categorical_target = ['SchemeName']
...
preprocessor = ColumnTransformer(
    transformers=[
        ...
        ('target', TargetEncoder(target_type='continuous', random_state=42), categorical_target)
    ])
...
print(f"R2 Score with Target Encoding: {r2}")
print(f"MAE with Target Encoding: {mae}")

# Re-train on full dataset for saving
model_pipeline_v2.fit(X, y)

# Construct the path to the save the joblib file
joblib_file_path = os.path.join(script_dir, '..', 'model', 'model_v2.pkl')

joblib.dump(model_pipeline_v2, joblib_file_path)
